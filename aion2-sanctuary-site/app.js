...
const DUNGEON_IMAGE = {
  '루드라': 'assets/dungeons/rudra.png',
  '침식': 'assets/dungeons/erosion.png',
  '무스펠 보통': 'assets/dungeons/muspel.png',
  '무스펠 어려움': 'assets/dungeons/muspel.png',
};

const defaultCharacters = [
  { id: crypto.randomUUID(), name: '푸딩힐', className: '치유성', representative: true },
  { id: crypto.randomUUID(), name: '푸딩궁', className: '궁성', representative: false },
  { id: crypto.randomUUID(), name: '푸딩권', className: '권성', representative: false },
];

const defaultRecruits = [
  { id: 'r1', dungeon: '루드라', time: '오늘 20:30', current: 4, max: 6, roles: '딜러 1명 · 힐러 1명', leader: '데바수호자' },
  { id: 'r2', dungeon: '침식', time: '오늘 21:00', current: 5, max: 6, roles: '딜러 1명', leader: '아이온전사' },
  { id: 'r3', dungeon: '무스펠 보통', time: '오늘 21:30', current: 3, max: 6, roles: '탱커 1명 · 힐러 2명', leader: '빛의수호자' },
  { id: 'r4', dungeon: '무스펠 어려움', time: '오늘 22:30', current: 5, max: 6, roles: '딜러 1명', leader: '지배' },
  { id: 'r5', dungeon: '루드라', time: '오늘 23:00', current: 2, max: 6, roles: '직업 무관 4명', leader: '푸딩' },
];

const state = {
  characters: load('aion2_characters', defaultCharacters),
  recruits: load('aion2_recruits', defaultRecruits),
  applications: load('aion2_applications', {}),
  activities: load('aion2_activities', [
    { text: '아이온전사님이 침식 파티에 참여했습니다.', time: '5분 전' },
    { text: '빛의수호자님이 루드라 파티에 참여했습니다.', time: '15분 전' },
    { text: '암흑마도사님이 무스펠 어려움 파티를 취소했습니다.', time: '32분 전' },
  ]),
  filter: '전체',
...
function save() {
  localStorage.setItem('aion2_characters', JSON.stringify(state.characters));
  localStorage.setItem('aion2_recruits', JSON.stringify(state.recruits));
  localStorage.setItem('aion2_applications', JSON.stringify(state.applications));
  localStorage.setItem('aion2_activities', JSON.stringify(state.activities));
}
...
function renderCounts() {
  ...
  document.querySelector('#count-rudra').textContent = count('루드라');
  document.querySelector('#count-erosion').textContent = count('침식');
  document.querySelector('#count-muspel-normal').textContent = count('무스펠 보통');
  document.querySelector('#count-muspel-hard').textContent = count('무스펠 어려움');
}
function renderRecruits() {
  ...
  if (!rows.length) {
    list.innerHTML = '<div class="empty">현재 모집 중인 파티가 없습니다.</div>';
    return;
  }
  list.innerHTML = rows.map(r => {
    ...
    return `<article class="recruit-item">
      ...
      <div><div class="label">필요 인원</div><div class="value">${escapeHtml(r.roles)}</div></div>
...
function renderCharacters() {
  ...
  if (!state.characters.length) {
    wrap.innerHTML = '<div class="empty">등록한 캐릭터가 없습니다.</div>';
    return;
  }
...
function confirmJoin() {
  ...
  recruit.current = Math.min(recruit.max, recruit.current + 1);
...
function openCharacterModal() {
  ...
  openModal(`<span class="modal-eyebrow">내 캐릭터</span><h2 id="modal-title">캐릭터 추가</h2><p class="modal-desc">캐릭터명과 직업을 등록하세요.</p><div class="form-grid">
    <div class="field"><label for="character-name">캐릭터명</label><input id="character-name" maxlength="20" placeholder="예: 푸딩힐" /></div>
...
function openRecruitModal() {
  ...
  openModal(`<span class="modal-eyebrow">새 모집</span><h2 id="modal-title">성역 모집 만들기</h2><p class="modal-desc">필수 정보만 간단하게 입력하세요.</p><div class="form-grid">
    <div class="field"><label>성역</label><select id="new-dungeon">${dungeonOptions}</select></div>
    <div class="field"><label>시간</label><input id="new-time" value="오늘 22:00" /></div>
    <div class="field"><label>현재 인원</label><input id="new-current" type="number" min="1" max="6" value="1" /></div>
    <div class="field"><label>총 인원</label><input id="new-max" type="number" min="2" max="12" value="6" /></div>
    <div class="field"><label>필요 인원</label><input id="new-roles" value="딜러 2명 · 힐러 1명" /></div>
    <div class="field"><label>파티장</label><input id="new-leader" value="푸딩" /></div>
  ...
  modalContent.querySelector('#save-recruit').addEventListener('click', () => {
    ...
    const max = Number(modalContent.querySelector('#new-max').value || 6);
